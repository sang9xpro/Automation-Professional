...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_COMMENT: 'comment/PARTIAL_UPDATE_COMMENT',
...
export default (state: CommentState = initialState, action): CommentState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_COMMENT):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_COMMENT):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_COMMENT):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const partialUpdate: ICrudPutAction<IComment> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_COMMENT,
