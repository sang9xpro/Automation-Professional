...
/**
 * REST controller for managing {@link com.automation.professional.domain.Country}.
 */
@RestController
@RequestMapping("/api")
public class CountryResource {
    ...
    public ResponseEntity<List<CountryDTO>> getAllCountries(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("devices-is-null".equals(filter)) {
            log.debug("REST request to get all Countrys where devices is null");
            return new ResponseEntity<>(countryService.findAllWhereDevicesIsNull(), HttpStatus.OK);
        }
        if ("accounts-is-null".equals(filter)) {
            log.debug("REST request to get all Countrys where accounts is null");
            return new ResponseEntity<>(countryService.findAllWhereAccountsIsNull(), HttpStatus.OK);
        }
        if ("facebook-is-null".equals(filter)) {
            log.debug("REST request to get all Countrys where facebook is null");
            return new ResponseEntity<>(countryService.findAllWhereFacebookIsNull(), HttpStatus.OK);
        }
